...
class ShoeController {
  ...
  addToCart(req, res, next) {
    ...
    Shoe.findById(shoeId, function (err, shoe) {
      ...
      if (shoe.sale > 0) {
        var newPrice = (shoe.price * (100 - shoe.sale)) / 100;
        saleShoe = new Shoe({
          brand: shoe.brand,
          type: shoe.type,
          name: shoe.name,
    ...
    if (!req.cookies.token) {
      Promise.all([
        ...
        Shoe.findOne({ _id: req.query.shoeid })
          ...
          .populate("type")
          .limit(4),
      ]).then(([brandList, shoeType, shoe, shoeDetail]) => {
      ...
      Promise.all([
        ...
        Notification.find({ user: decodeToken })
          // .limit(4)
          .sort({ createdAt: -1 }),
      ]).then(([data, brandList, shoeType, shoe, shoeDetail, noti]) => {
        if (data) {
          req.data = data;
  ...
  storeCustomToCart(req, res, next) {
    ...
    Custom.findById(customId, function (err, custom) {
      ...
      var id = custom._id + shoeSize;
      cart.add(custom, id, shoeSize);
      req.session.cart = cart;
      res.redirect("back");
    });
  ...
  newarrival(req, res, next) {
    if (!req.cookies.token) {
      Promise.all([
        Brand.find({}),
        Shoetype.find({}),
        Shoe.find({}).populate("brand").populate("type").sort({ createdAt: 1 }),
      ]).then(([brandList, shoeType, shoe]) => {
        res.render("shoe", {
          brandList: multipleMongooseToObject(brandList),
          shoeType: multipleMongooseToObject(shoeType),
          shoe: multipleMongooseToObject(shoe),
      ...
      Promise.all([
        ...
        Notification.find({ user: decodeToken })
          // .limit(4)
          .sort({ createdAt: -1 }),
      ]).then(([data, brandList, shoeType, shoe, noti]) => {
        if (data) {
          req.data = data;
  ...
  cheapshoe(req, res, next) {
    if (!req.cookies.token) {
      Promise.all([
        Brand.find({}),
        Shoetype.find({}),
        Shoe.find({}).populate("brand").populate("type").sort({ price: 1 }),
      ]).then(([brandList, shoeType, shoe]) => {
        res.render("shoe", {
          brandList: multipleMongooseToObject(brandList),
          shoeType: multipleMongooseToObject(shoeType),
  ...
  bestseller(req, res, next) {
    if (!req.cookies.token) {
      Promise.all([
        Brand.find({}),
        Shoetype.find({}),
        Shoe.find({ bestseller: true }).populate("brand").populate("type"),
      ]).then(([brandList, shoeType, shoe]) => {
        res.render("shoe", {
          brandList: multipleMongooseToObject(brandList),
          shoeType: multipleMongooseToObject(shoeType),
  ...
  available(req, res, next) {
    if (!req.cookies.token) {
      Promise.all([
        Brand.find({}),
        Shoetype.find({}),
        Shoe.find({ available: true }).populate("brand").populate("type"),
      ]).then(([brandList, shoeType, shoe]) => {
        res.render("shoe", {
          brandList: multipleMongooseToObject(brandList),
          shoeType: multipleMongooseToObject(shoeType),
